...
reserveBtn.onclick = async () => {
  if (!currentUser) return alert("로그인 먼저 해주세요.");
  if (!selectedSeat || !reserveDate.value) return alert("좌석과 예약 날짜를 선택해주세요.");
  const selectedSessions = ["lunch", "dinner", "part1", "part2"].filter((name) => document.getElementById(`${name}Check`)?.checked);
  if (!selectedSessions.length) return alert("사용할 시간을 하나 이상 선택해주세요.");
  try {
    await reserveSeatCall({ date: reserveDate.value, seatNum: selectedSeat, selectedSessions });
    popup.classList.add("hidden");
    selectedSeat = null;
    [lunchCheck, dinnerCheck, part1Check, part2Check].forEach((check) => { check.checked = false; });
    alert("예약이 완료되었습니다.");
  } catch (error) {
    console.error(error);
...
openCancelReservationPopup = (date, seatNum, times) => {
  const labels = { lunch: "점심", dinner: "저녁", part1: "야자 1부", part2: "야자 2부" };
  cancelTimeButtons.innerHTML = "";
  for (const [session, label] of Object.entries(labels)) {
    if (times[session]?.owner !== currentUser) continue;
    const button = document.createElement("button");
    button.textContent = `${label} 예약 취소`;
    button.onclick = async () => {
      if (!confirm(`${label} 예약을 취소하시겠습니까?`)) return;
      try {
        await cancelReservationCall({ date, seatNum, session });
        cancelReservationPopup.classList.add("hidden");
        alert("예약이 취소되었습니다.");
      } catch (error) { console.error(error); alert(error.message || "예약 취소에 실패했습니다."); }
    };
    cancelTimeButtons.appendChild(button);
  }
  cancelReservationPopup.classList.remove("hidden");
